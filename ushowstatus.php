 ...
 $em=$_COOKIE["Email"];
 $status=mysqli_query($con,"select s.FIRSTNAME,s.LASTNAME,s.PIC,s.EMAIL,st.STATUS,st.STATUSID,st.DATE,st.TIME,st.EMAIL from status st,signup s where s.EMAIL=st.EMAIL and st.EMAIL='$userem' order by st.STATUSID desc");
$i=0;
...
<button type="button" onclick="comdit('<?php echo $showsc["COMMENTID"];?>','<?php echo $i; ?>')" class="btnpost" >Update</button>
