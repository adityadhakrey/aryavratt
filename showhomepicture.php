 ...
 $em=$_COOKIE["Email"];
 $pid=$row["PICTUREID"];
 $picture=mysqli_query($con,"select (select PIC from signup where EMAIL=h.EMAIL) as PIC,(select FIRSTNAME from signup where EMAIL=h.EMAIL) as FIRSTNAME,(select LASTNAME from signup where EMAIL=h.EMAIL) as LASTNAME,h.* from home h where (h.EMAIL='$em' or h.EMAIL in(select FRIENDEMAIL from friend where EMAIL='$em' and FRIENDEMAIL<>'$em')) and h.TYPE='picture' and h.PICTUREID=$pid");
 //$i=0;
 $i=$pid;
...
<?php
  ...
  <!--modal end-->
          ...
          <h6 class="w3-text-teal"><i class="fa fa-calendar fa-fw w3-margin-right"></i><?php echo $showpicture["DATE"];?> - <span class="w3-tag w3-teal w3-round"><?php echo $showpicture["TIME"];?></span></h6>
		  
		  <?php
 if($em==$showpicture["EMAIL"])
 {
	 ?>
		  <h6 class="w3-text-teal"><a  onclick="openpicedit('<?php echo $i;?>');" onmouseout="this.style.color='white'"onmouseover="this.style.color ='orange'" style="cursor:pointer;color:white;"><i class="fa fa-edit fa-fw"></i>Edit</a>
		  <a onclick="deletepicture('<?php echo $showpicture["PICTUREID"];?>')" onmouseout="this.style.color='white'"onmouseover="this.style.color ='orange'" style="cursor:pointer;color:white;"><i class="fa fa-trash fa-fw"></i>Delete</a></h6>
<?php
 }
?> 
	   <p><?php echo $showpicture["PICTURECAPTION"];?>
