 ...
 $em=$_COOKIE["Email"];
 $stid=$row["STATUSID"];
 //$status=mysqli_query($con,"select s.FIRSTNAME,s.LASTNAME,s.PIC,h.EMAIL,h.STATUS,h.STATUSID,h.DATE,h.TIME from home h,signup s where h.EMAIL='$em' or h.EMAIL in(select FRIENDEMAIL from friend where EMAIL='$em' and FRIENDEMAIL<>'$em') and h.TYPE='status' order by h.HOMEID desc");
 $status=mysqli_query($con,"select (select PIC from signup where EMAIL=h.EMAIL) as PIC,(select FIRSTNAME from signup where EMAIL=h.EMAIL) as FIRSTNAME,(select LASTNAME from signup where EMAIL=h.EMAIL) as LASTNAME,h.* from home h where (h.EMAIL='$em' or h.EMAIL in(select FRIENDEMAIL from friend where EMAIL='$em' and FRIENDEMAIL<>'$em')) and h.TYPE='status' and h.STATUSID=$stid");
 $i=$stid;
...
<?php
  ...
  <!--modal end-->
          ...
          <h6 class="w3-text-teal"><i class="fa fa-calendar fa-fw w3-margin-right"></i><?php echo $showstatus["DATE"];?> - <span class="w3-tag w3-teal w3-round"><?php echo $showstatus["TIME"];?></span></h6> 
<?php
if($em==$showstatus["EMAIL"])
{
?>	
		  
	<h6 class="w3-text-teal"><a onclick="openstatusedit('<?php echo $i;?>')" onmouseout="this.style.color='white'"onmouseover="this.style.color ='orange'" style="cursor:pointer;color:white;"><i class="fa fa-edit fa-fw"></i>Edit</a>
...
<div class="input-con">
<input type="text"  id="statuscomment<?php echo $i; ?>"  style="color:black;" placeholder="Write Comment....." class="input-field">
