<div class="scrollbar" id="proscr" onscroll="proscrl()" style="height:700px;overflow-y:auto;">
<div class="w3-container" style="color:white;padding-left:2%;padding-right:2%;">
<?php
 include("connection.php");
 $userem=$_REQUEST["userem"];
 $em=$_COOKIE["Email"];
 $project=mysqli_query($con,"select s.FIRSTNAME,s.LASTNAME,s.PIC,s.EMAIL,p.PROJECT,p.DATE,p.TIME,p.TITLE,p.SUMMARY,p.CATEGORY,p.EMAIL,p.PROJECTID from project p,signup s where s.EMAIL=p.EMAIL and p.EMAIL='$userem' order by p.PROJECTID desc");
$i=0;
...
<hr>		  
  ...
  <!--modal end-->
		  ...
		  </br></br><a onclick="downloadproject('<?php echo $showpro["PROJECTID"];?>')" href="userproject/<?php echo $showpro["PROJECT"];?>" download ><button  type="button" class="btndwn"><i style="width:100%;" class="fa fa-download"></i>Download This Project</button></a>
		  <?php 
...
<?php
		  ...
		  {
		  ?>
</br></br><button  onclick="book('<?php echo $showpro["PROJECTID"];?>','<?php echo $userem;?>')" type="button" class="btndwn"><i style="width:100%;" class="fa fa-bookmark"></i>add to bookmark</button> 
<?php
		  }
		  ?>
