 ...
 $em=$_COOKIE["Email"];
 $ssid=$n["STATUSID"];
 $status=mysqli_query($con,"select s.FIRSTNAME,s.LASTNAME,s.PIC,s.EMAIL,st.STATUS,st.STATUSID,st.DATE,st.TIME,st.EMAIL from status st,signup s where s.EMAIL=st.EMAIL and st.STATUSID='$ssid' ");
$i=0;
...
<?php
  ...
  <!--modal end-->
          ...
          <h6 class="w3-text-teal"><i class="fa fa-calendar fa-fw w3-margin-right"></i><?php echo $showstatus["DATE"];?> - <span class="w3-tag w3-teal w3-round"><?php echo $showstatus["TIME"];?></span>&nbsp <a  onclick="openedit(<?php echo $i; ?>)" onmouseout="this.style.color='white'"onmouseover="this.style.color ='orange'" style="cursor:pointer;color:white;"><i class="fa fa-edit fa-fw"></i>Edit</a>
...
<textarea  style="resize:none;width:100%;color:black;" id="editstatus<?php echo $i;?>"  rows="5" placeholder="Edit Here.." required=""><?php echo $showstatus["STATUS"];?></textarea>
<button type="button" onclick="statusedit('<?php echo $showstatus["STATUSID"];?>')" class="btnpost" >Update</button>
