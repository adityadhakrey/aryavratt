 ...
 $em=$_COOKIE["Email"];
 $picture=mysqli_query($con,"select s.FIRSTNAME,s.LASTNAME,s.PIC,s.EMAIL,p.PICTURE,p.PICTURECAPTION,p.EMAIL,p.PICTUREID,p.DATE,p.TIME from picture p,signup s where s.EMAIL=p.EMAIL and (p.EMAIL='$userem' and s.EMAIL<>'$em') order by p.PICTUREID desc");
 $i=0;
...
<?php
  ...
  <!--modal end-->
          ...
          <h6 class="w3-text-teal"><i class="fa fa-calendar fa-fw w3-margin-right"></i><?php echo $showpicture["DATE"];?> - <span class="w3-tag w3-teal w3-round"><?php echo $showpicture["TIME"];?></span>&nbsp </h6>
